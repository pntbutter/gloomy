...
<template>
    <div class="flex  flex-col  gap-2  p-4  bg-slate-800">
        ...
        <div v-if="show_edit" class="fixed  z-40  inset-0  flex  flex-col  gap-3  p-6  bg-slate-800/70  backdrop-blur-md  border  border-slate-800">
            ...
            <div class="flex  flex-wrap  gap-4">
                <button type="button" v-for="(unit, index) in enemy.units" @click="remove_unit(index)" class="relative  overflow-hidden  grid  place-content-center  w-14  h-14  rounded  text-xs  text-white" :class="unit.elite ? 'bg-amber-300' : 'bg-white'">
                    ...
                    <div class="absolute  right-0  bottom-0  w-5  h-4  rounded-tl  bg-rose-600">{{ unit.hp }}</div>
                    <IconClose class="text-black" />
                </button>
            </div>

            <div class="flex  gap-4  mt-2">
                <button type="button" @click="add_normal" class="flex  items-center  gap-2  px-4  py-2.5  rounded  bg-slate-700">
                    <IconPlus /> Add normal unit
                </button>

                <button type="button" @click="add_elite" class="flex  items-center  gap-2  px-4  py-2.5  rounded  bg-slate-700">
